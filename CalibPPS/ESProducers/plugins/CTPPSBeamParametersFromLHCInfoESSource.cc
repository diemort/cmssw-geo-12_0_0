...
std::unique_ptr<CTPPSBeamParameters> CTPPSBeamParametersFromLHCInfoESSource::produce(const CTPPSBeamParametersRcd &iRecord)
{
  ...
  const auto beamMom = hLHCInfo->energy();
  const auto betaStar = hLHCInfo->betaStar() * 1E2; // conversion m --> cm
  const auto xangle = hLHCInfo->crossingAngle() * 1E-6; // conversion mu rad --> rad
