...
std::shared_ptr<LHCInterpolatedOpticalFunctionsSetCollection> CTPPSModifiedOpticalFunctionsESSource::produce(const CTPPSInterpolatedOpticsRcd& iRecord)
{
  ...
  // prepare output
  std::shared_ptr<LHCInterpolatedOpticalFunctionsSetCollection> output = std::make_shared<LHCInterpolatedOpticalFunctionsSetCollection>(*hInput);
  ...
  for (const auto &fsp : *output)
  {
    ...
    if (rpDecId == rpDecId_45_N_) armInfo[0].rpId_N = fsp.first;
    if (rpDecId == rpDecId_45_F_) armInfo[0].rpId_F = fsp.first;
    if (rpDecId == rpDecId_56_N_) armInfo[1].rpId_N = fsp.first;
    if (rpDecId == rpDecId_56_F_) armInfo[1].rpId_F = fsp.first;
  }
